...
Future<void> killProcessOnPort(int port) async {
  ...
  if (result.exitCode != 0 || result.stdout.toString().trim().isEmpty) {
    logger.e('No process found running on port $port');
    return;
  }

  final pid = result.stdout.toString().trim();
  logger.i('Killing process with PID $pid on port $port');

  final killResult = await Process.run('kill', ['-9', pid]);
  if (killResult.exitCode == 0) {
    logger.i('Process killed.');
  } else {
    logger.e('Failed to kill process: ${killResult.stderr}');
  }
}
                ...
                if (!urlLaunched) {
                  ...
                  if (urlMatch != null) {
                    ...
                    if (url != null) {
                      urlLaunched = true;
                      launchUrlString(url);
                      logger.i('Auth URL launched: $url');
                    }
                  }
                }
              });
