...
Future<void> _showBackgroundNotification() async {
  ...
  const AndroidNotificationDetails androidNotificationDetails =
      AndroidNotificationDetails(
        'sync_notif',
        'WFL Codex 网盘',
        channelDescription: '后台同步状态通知',
        importance: Importance.min,
        priority: Priority.min,
        silent: true,
        ticker: 'WFL Codex 正在后台同步',
        actions: [AndroidNotificationAction('cancel_sync', '停止后台同步')],
      );
  ...
  Workmanager().executeTask((task, inputData) async {
    ...
    for (final connection in connections) {
      ...
      if (firstFolderOpt.isNone() || secondFolderOpt.isNone()) {
        logger.i('First or second folder is none');
        continue;
      }
      ...
      if (firstProvider == null || secondProvider == null) {
        logger.e('One/Both providers are null');
        continue;
      }

      await localFolder.match(
        () {
          logger.e('Local folder does not exist');
        },
        (localFolder) async {
          ...
          try {
            ...
            logger.e('Failed to fetch local files', error: err);
          }
...
void notificationHandler(NotificationResponse notificationResponse) async {
  if (notificationResponse.actionId == 'cancel_sync') {
    ...
    logger.i('Cancelled all background tasks');
  }
}

void main() async {
  logger.i('Initialize bindings');
  SentryWidgetsFlutterBinding.ensureInitialized();
  // WidgetsFlutterBinding.ensureInitialized();

  // Hive stuff
  logger.i('Initialize Hive database');
  await Hive.initFlutter();
  Hive.registerAdapters();
  ...
  logger.i('Setup Hive boxes/tables');
  await setupHiveBox<SettingsModel>(boxPath);
  await setupHiveBox<IntroSettingsModel>(boxPath);
  await setupHiveBox<RemoteProviderModel>(boxPath);
  await setupHiveBox<ConnectionModel>(boxPath);
