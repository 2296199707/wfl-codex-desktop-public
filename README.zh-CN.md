...
当前版本：`v0.44.80-beta`。版本变化见 [CHANGELOG.md](CHANGELOG.md)。
...
- `git describe --tags --exact-match` 必须显示当前发布标签 `v0.44.80-beta`。
...
```bash
cd /root/install
sha256sum -c wfl-codex-desktop-v0.44.80-beta.tar.gz.sha256
tar -xzf wfl-codex-desktop-v0.44.80-beta.tar.gz -C /srv
mv /srv/wfl-codex-desktop-v0.44.80-beta /srv/wfl-codex-desktop
cd /srv/wfl-codex-desktop
sudo bash install.sh
```
