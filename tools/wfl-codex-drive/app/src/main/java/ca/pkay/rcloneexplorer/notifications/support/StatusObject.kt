...
class StatusObject(var mContext: Context){
    ...
    fun getPercentage(): Double {
        ...
        return (mStats.optLong("bytes", 0).toDouble() / totalBytes * 100)
            .coerceIn(0.0, 100.0)
    }
    ...
    fun parseLoglineToStatusObject(logLine: JSONObject) {
        ...
        if(logLine.has("stats")) {
            ...
            if(checks != null && checks.length() > 0) {
                var filename = checks.optString(0)
                if(!filename.equals("")) {
            ...
            val transferringItems = mStats.optJSONArray("transferring")
            if(transferringItems != null && transferringItems.length() > 0) {
                val transferring = transferringItems.getJSONObject(0)
