...
public class MainActivity extends AppCompatActivity
    ...
    private static final String WFL_REMOTE_NAME = "wfl-codex";
    private static final String WFL_SETUP_PROMPTED = "wfl_codex_setup_prompted";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (BuildConfig.WFL_CODEX_BUILD) {
            new Handler(Looper.getMainLooper()).postDelayed(this::maybeOfferWflLogin, 500);
        }
    }

    /**
     * WFL's WebDAV root is the authenticated account boundary. The first-run
     * dialog only creates a normal rclone WebDAV remote; project visibility and
     * all write permissions remain enforced by the server.
     */
    private void maybeOfferWflLogin() {
        if (isFinishing() || isDestroyed()) return;
        SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(this);
        if (rclone.getRemoteItemFromName(WFL_REMOTE_NAME) != null
                || preferences.getBoolean(WFL_SETUP_PROMPTED, false)) {
            return;
        }
        showWflLoginDialog();
    }

    private void showWflLoginDialog() {
        ...
        explanation.setText(getString(R.string.wfl_login_description));
        explanation.setPadding(0, 0, 0, horizontalPadding / 2);
        form.addView(explanation);

        EditText urlInput = new EditText(this);
        urlInput.setSingleLine(true);
        urlInput.setInputType(InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_URI);
        urlInput.setHint(R.string.wfl_login_url_hint);
        urlInput.setText(BuildConfig.WFL_DAV_URL);
        form.addView(urlInput, new LinearLayout.LayoutParams(-1, -2));

        EditText usernameInput = new EditText(this);
        usernameInput.setSingleLine(true);
        usernameInput.setInputType(InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_EMAIL_ADDRESS);
        usernameInput.setHint(R.string.wfl_login_username_hint);
        form.addView(usernameInput, new LinearLayout.LayoutParams(-1, -2));

        EditText passwordInput = new EditText(this);
        passwordInput.setSingleLine(true);
        passwordInput.setInputType(InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_PASSWORD);
        passwordInput.setHint(R.string.wfl_login_password_hint);
        form.addView(passwordInput, new LinearLayout.LayoutParams(-1, -2));

        AlertDialog dialog = new MaterialAlertDialogBuilder(this)
                .setTitle(R.string.wfl_login_title)
                .setView(form)
                .setNegativeButton(R.string.cancel, (ignored, which) -> PreferenceManager
                        .getDefaultSharedPreferences(this).edit().putBoolean(WFL_SETUP_PROMPTED, true).apply())
                .setPositiveButton(R.string.wfl_login_connect, null)
                .create();
        ...
        dialog.setOnShowListener(ignored -> dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(view -> {
            ...
            if (!isValidWflDavUrl(url) || username.isEmpty() || password.isEmpty()) {
                Toasty.error(this, getString(R.string.wfl_login_invalid), Toast.LENGTH_SHORT, true).show();
                return;
            }

            dialog.getButton(AlertDialog.BUTTON_POSITIVE).setEnabled(false);
            dialog.getButton(AlertDialog.BUTTON_NEGATIVE).setEnabled(false);
            Toasty.info(this, getString(R.string.wfl_login_connecting), Toast.LENGTH_SHORT, true).show();
            new Thread(() -> {
                ArrayList<String> options = new ArrayList<>();
                options.add(WFL_REMOTE_NAME);
                options.add("webdav");
                options.add("url");
                options.add(url);
                options.add("vendor");
                ...
                if (success) {
                    // Creating an rclone profile only validates its syntax. Do
                    // one authenticated listing before reporting success so a
                    // bad password can never leave a broken remote behind.
                    RemoteItem configuredRemote = rclone.getRemoteItemFromName(WFL_REMOTE_NAME);
                    success = configuredRemote != null
                            && rclone.getDirectoryContent(
                                    configuredRemote,
                                    "//" + WFL_REMOTE_NAME,
                                    true
                            ) != null;
                }
                if (!success) removeWflRemoteConfiguration();
                boolean configured = success;
                runOnUiThread(() -> {
                    if (configured) {
                        dialog.dismiss();
                        PreferenceManager.getDefaultSharedPreferences(this).edit()
                                .putBoolean(WFL_SETUP_PROMPTED, true).apply();
                        Toasty.success(this, getString(R.string.wfl_login_success), Toast.LENGTH_SHORT, true).show();
                        startRemotesFragment();
                    } else {
                        dialog.getButton(AlertDialog.BUTTON_POSITIVE).setEnabled(true);
                        dialog.getButton(AlertDialog.BUTTON_NEGATIVE).setEnabled(true);
                        Toasty.error(this, getString(R.string.wfl_login_failed), Toast.LENGTH_LONG, true).show();
                    }
                });
            }).start();
        }));
    ...
    private boolean isValidWflDavUrl(String value) {
        try {
    ...
    private void removeWflRemoteConfiguration() {
        Process cleanup = rclone.config("delete", new ArrayList<>(Arrays.asList(WFL_REMOTE_NAME)));
