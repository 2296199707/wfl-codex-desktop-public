...
class EphemeralTaskManager(private var mContext: Context) {
    ...
    protected fun work(inputData: Data, tag: String) {
        ...
        if (inputData.getString(EphemeralWorker.EPHEMERAL_TYPE) == Type.UPLOAD.name) {
            // Start user-initiated uploads promptly. If expedited quota is
            // exhausted, WorkManager falls back to a regular request and the
            // worker still promotes itself to a foreground service.
            uploadWorkRequest.setExpedited(OutOfQuotaPolicy.RUN_AS_NON_EXPEDITED_WORK_REQUEST)
        }
