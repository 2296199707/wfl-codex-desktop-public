...
class UpdateChecker(private var mContext: Context) {
    ...
    // The upstream checker points at Round Sync releases. A WFL build must
    // never offer an APK with a different application identity or signature.
    private var checkForUpdates = !BuildConfig.WFL_CODEX_BUILD
            && preferenceManager.getBoolean(mContext.getString(R.string.pref_key_app_updates), false)
