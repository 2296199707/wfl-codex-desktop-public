...
This directory contains the WFL Codex fork of [Round Sync](https://github.com/newhinton/Round-Sync),
an Android file manager and synchronization client based on rclone.

## WFL Codex connection

On the first launch, the app opens a WFL Codex WebDAV login form. Enter the HTTPS `/dav`
endpoint of the deployment you use and the same username and password used on that website.
The server returns only the authenticated account's project directories. Project permissions,
quotas, and the protected project roots remain enforced server-side.

The client keeps Round Sync's mature file browser, upload/download queue, background tasks,
folder management, previews, sync jobs, and Android Storage Access Framework support. See
[`NOTICE-WFL-CODEX.md`](NOTICE-WFL-CODEX.md) for attribution and licensing.

## Build the WFL fork

Requirements: Java 17, Android SDK/NDK, and Go 1.24+. Supply a WFL release keystore outside
the repository; the upstream signing key is not included.

```bash
export ANDROID_HOME=/opt/android-sdk
export PATH=/path/to/go/bin:$PATH
./gradlew assembleOssRelease \
  -PwflDavUrl='https://your-private-domain.example/dav' \
  -PwflKeystore=/secure/wfl-codex-drive.keystore \
  -PwflKeyAlias=wfl-codex-drive \
  -PwflStorePassword='…' \
  -PwflKeyPassword='…'
```

The `oss` flavor is the redistributable build. Keep the GPL-3.0 license and corresponding
source when distributing the APK.
