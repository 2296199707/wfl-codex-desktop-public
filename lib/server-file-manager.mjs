...
export function parseServerFileRange(value, size) {
  if (value == null || value === "") return null;
  if (typeof value !== "string" || !Number.isSafeInteger(size) || size < 0) {
    throw fileManagerError(416, "文件下载范围无效", "SERVER_FILE_RANGE_INVALID");
  }
  const match = /^bytes=(\d*)-(\d*)$/u.exec(value.trim());
  if (!match || (!match[1] && !match[2]) || size === 0) {
    throw fileManagerError(416, "文件下载范围无法满足", "SERVER_FILE_RANGE_INVALID");
  }
...
export function serverFileDownloadTag(stat) {
