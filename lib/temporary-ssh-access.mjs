...
import { temporarySshCommandArguments } from "./temporary-ssh-command.mjs";
...
export class TemporarySshAccessService {
  ...
  async execute(id, command, { timeoutMs = DEFAULT_COMMAND_TIMEOUT_MS } = {}) {
    this.assertPrimary();
    const record = this.requireActiveRecord(id);
    ...
    return new Promise((resolve, reject) => {
      ...
      const cleanup = () => {
        const processes = this.activeProcesses.get(record.id);
        processes?.delete(child);
        if (processes && processes.size === 0) this.activeProcesses.delete(record.id);
      };
      ...
      const stop = () => {
        try {
          ...
          // The command may have already exited; its original result wins.
        }
      };
      const append = (target, chunk) => {
        ...
        if (outputBytes > MAX_OUTPUT_BYTES) {
          truncated = true;
          finish(resolve, commandResult(record, {
            stdout,
            stderr,
            truncated,
            exitCode: null,
      ...
      try {
        child = this.spawnCommand("ssh", [...temporarySshCommandArguments(record), normalizedCommand], {
          env: sshCommandEnvironment(),
          stdio: ["ignore", "pipe", "pipe"],
        });
        const processes = this.activeProcesses.get(record.id) || new Set();
        processes.add(child);
        this.activeProcesses.set(record.id, processes);
        child.stdout.setEncoding("utf8");
        child.stderr.setEncoding("utf8");
        child.stdout.on("data", (chunk) => append("stdout", chunk));
        child.stderr.on("data", (chunk) => append("stderr", chunk));
        child.once("error", (error) => finish(reject, sshCommandError(error)));
        child.once("close", (exitCode, signal) => finish(resolve, commandResult(record, {
          stdout,
          stderr,
          truncated,
          exitCode,
        ...
        timer = setTimeout(() => {
          finish(reject, accessError(504, "SSH 远程命令超时"));
          stop();
        }, normalizedTimeout);
        timer.unref?.();
      } catch (error) {
        finish(reject, sshCommandError(error));
      }
    });
  }
  ...
  get busy() {
    return this.pendingOperationCount > 0 || this.activeCommandCount > 0;
  }

  get activeCommandCount() {
    let count = 0;
    for (const processes of this.activeProcesses.values()) count += processes.size;
    return count;
  }
  ...
  async revokeNow(id) {
    ...
    this.stopActiveProcesses(record.id);
  ...
  stopActiveProcesses(id) {
    const processes = this.activeProcesses.get(String(id));
    if (!processes) return;
    for (const child of processes) {
      try {
        if (!child.killed) child.kill("SIGTERM");
      } catch {
        // Process cleanup is best effort; authorization removal still proceeds.
      }
    }
  }

  async close() {
    for (const id of this.activeProcesses.keys()) this.stopActiveProcesses(id);
  }

  requireActiveRecord(id) {
    const record = this.records.get(String(id || ""));
    if (!record || record.expiresAt <= this.now()) {
      throw accessError(404, "临时 SSH 授权不存在或已经到期");
    }
    return record;
  }
...
function normalizeCommand(value) {
  ...
  if (!command || command.length > MAX_COMMAND_LENGTH || /[\u0000\r\n]/.test(command)) {
    throw accessError(400, "远程命令不能为空，且不能包含换行或控制字符");
  }
  return command;
}
...
function commandResult(record, { stdout, stderr, truncated, exitCode, signal }) {
  return {
    id: record.id,
...
function sshCommandError(error) {
  if (error?.code === "ENOENT") return accessError(503, "本机缺少 OpenSSH 客户端");
  return accessError(502, "SSH 远程命令无法启动");
}

function sshCommandEnvironment() {
  return {
