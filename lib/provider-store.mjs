...
const IMAGE_API_REQUEST_MODES = new Set(["managed", "partial", "passthrough"]);
...
function imageApiConfigurationRevision(imageApi, provider) {
  ...
  return revisionHash({
    ...
    requestMode: imageApi.requestMode || "managed",
...
function normalizeImageApi(input, existing = null, { stored = false } = {}) {
  ...
  const requestMode = normalizeImageRequestMode(input, existing);
...
function normalizeImageRequestMode(input, existing) {
  const value = Object.hasOwn(input || {}, "requestMode")
    ? String(input.requestMode || "").trim()
    : String(existing?.requestMode || "managed").trim();
  if (!IMAGE_API_REQUEST_MODES.has(value)) throw storeError(400, "不支持这个图片请求参数模式");
  return value;
}
