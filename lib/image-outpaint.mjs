...
export async function restoreOutpaintProviderCanvas(resultBuffer, plan) {
  const transform = normalizeCanvasTransform(plan);
  const input = normalizeBuffer(resultBuffer, "outpaint result");
  const metadata = await sharp(input, {
    failOn: "error",
    limitInputPixels: MAX_CANVAS_PIXELS,
  }).metadata();
  const needsResize = metadata.width !== transform.providerWidth || metadata.height !== transform.providerHeight;
  if (transform.kind === "none" && !needsResize) return input;
  let pipeline = sharp(input, {
    failOn: "error",
    limitInputPixels: MAX_CANVAS_PIXELS,
  });
  if (needsResize) {
    pipeline = pipeline.resize(transform.providerWidth, transform.providerHeight, {
      fit: "fill",
      kernel: sharp.kernel.lanczos3,
    });
  }
  if (transform.kind === "none") return pipeline.png({ compressionLevel: 9 }).toBuffer();
