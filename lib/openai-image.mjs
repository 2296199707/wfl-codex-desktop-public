...
function normalizedRequestError(error, abort) {
  ...
  if (!Number.isInteger(error?.statusCode)) {
    return imageError(502, "无法连接图片供应商", "IMAGE_PROVIDER_UNREACHABLE", {
      retryable: true,
      providerRequestId: abort?.providerRequestId ?? null,
      transportPhase: providerTransportPhase(error),
    });
  }
  return error;
}

function providerTransportPhase(error) {
  const codes = new Set([
    error?.code,
    error?.cause?.code,
    error?.cause?.cause?.code,
  ].filter((value) => typeof value === "string").map((value) => value.toUpperCase()));
  if (["ENOTFOUND", "EAI_AGAIN", "EAI_FAIL", "EAI_NODATA"].some((code) => codes.has(code))) return "dns";
  if ([
    "CERT_HAS_EXPIRED",
  ...
  ].some((code) => codes.has(code)) || [...codes].some((code) => code.startsWith("ERR_SSL_"))) return "tls";
  if (["ETIMEDOUT", "UND_ERR_BODY_TIMEOUT", "UND_ERR_CONNECT_TIMEOUT", "UND_ERR_HEADERS_TIMEOUT"].some((code) => codes.has(code))) {
    return "timeout";
  }
  if (["ECONNRESET", "EPIPE", "UND_ERR_SOCKET"].some((code) => codes.has(code))) return "reset";
  if (["ECONNREFUSED", "EHOSTUNREACH", "ENETUNREACH", "EADDRNOTAVAIL"].some((code) => codes.has(code))) return "connect";
  return "network";
}
