...
export async function requestProviderImages(options = {}) {
  ...
  try {
    ...
    const request = config.operation === "generate"
      ? generationRequest(config.apiKey, config.requested, abort.controller.signal)
      : editRequest({
...
function normalizeProviderOptions(options, streaming) {
  ...
  const expectedSize = parseExpectedSize(options.size);
  const expectedFormat = normalizeExpectedFormat(options.outputFormat);
  if (options.background === "transparent" && expectedFormat === "jpeg") {
    throw imageError(400, "透明背景只支持 PNG 或 WebP 输出", "INVALID_IMAGE_BACKGROUND");
  }
  const user = normalizeUser(options.user);
  const partialImages = streaming ? normalizePartialImages(options.partialImages) : null;
...
function normalizePartialImages(value) {
  ...
  if (!Number.isSafeInteger(value) || value < 0 || value > 3) {
    throw imageError(400, "流式局部图片数量必须为 0-3", "INVALID_PARTIAL_IMAGE_COUNT");
  }
  return value;
}
...
function generationRequest(apiKey, requested, signal) {
  return {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify(providerParameters(requested)),
    signal,
  };
}
...
function editRequest({
  ...
  for (const [name, value] of Object.entries(providerParameters(requested))) {
...
function providerParameters(requested) {
  return compactObject({
    ...
    output_format: requested.outputFormat,
    output_compression: requested.outputCompression,
    background: requested.background,
    moderation: requested.moderation,
    input_fidelity: requested.inputFidelity,
    stream: requested.stream,
    partial_images: requested.partialImages,
  });
}

function normalizeOutput(entry, { expectedSize, expectedFormat, maxOutputBytesPerImage }) {
  ...
  if (expectedSize && (details.width !== expectedSize.width || details.height !== expectedSize.height)) {
    throw imageError(502, "图片供应商返回的图片尺寸与请求不符", "IMAGE_SIZE_MISMATCH", {
      requestedWidth: expectedSize.width,
      requestedHeight: expectedSize.height,
      actualWidth: details.width,
      actualHeight: details.height,
    });
  }
  return {
    image,
