...
export class MultiUserStore {
  constructor(directory, {
    ...
    this.legacyProjectRoots = normalizeProjectRoots(
      legacyProjectRoots || [this.legacyProjectRoot],
      this.legacyProjectRoot,
    );
  ...
  async initialize({ writeOnInitialize = true } = {}) {
    ...
    const storedLegacyProjectRoots = legacyOwner && Array.isArray(legacyOwner.projectRoots)
      ? legacyOwner.projectRoots.map((root) => path.resolve(String(root)))
      : legacyOwner
        ? [path.resolve(String(legacyOwner.projectRoot || ""))]
        : [];
    if (legacyOwner && (
      legacyOwner.projectRoot !== this.legacyProjectRoot
      || legacyOwner.defaultProject !== this.legacyDefaultProject
      || JSON.stringify(storedLegacyProjectRoots) !== JSON.stringify(this.legacyProjectRoots)
    )) {
      legacyOwner.projectRoot = this.legacyProjectRoot;
      legacyOwner.projectRoots = [...this.legacyProjectRoots];
      legacyOwner.defaultProject = this.legacyDefaultProject;
    ...
    return this.mutate(async () => {
      ...
      if (!owner) {
        ...
        owner = {
          ...
          projectRoots: [...this.legacyProjectRoots],
  ...
  async updateLegacyProjectRoots(actorId, projectRoots, defaultProject) {
    return this.mutate(async () => {
      const actor = this.requireRole(actorId, ["owner", "admin"]);
      const roots = normalizeProjectRoots(projectRoots, this.legacyProjectRoot);
      const nextDefaultProject = path.resolve(String(defaultProject || ""));
      if (!projectRootForPath(roots, nextDefaultProject)) {
        throw storeError(409, "所有者默认工程必须位于项目存储根目录内");
      }
      this.legacyProjectRoot = roots[0];
      this.legacyProjectRoots = [...roots];
      this.legacyDefaultProject = nextDefaultProject;
      const owner = this.users.users.find((user) => user.id === this.config.ownerId);
      if (owner?.legacy) {
        owner.projectRoot = this.legacyProjectRoot;
        owner.projectRoots = [...this.legacyProjectRoots];
        owner.defaultProject = this.legacyDefaultProject;
        owner.updatedAt = this.now();
        await writeJsonAtomic(this.usersPath, this.users);
      }
      await this.audit("project_roots.updated", actor.id, {
        projectRoots: [...roots],
        defaultProject: nextDefaultProject,
      });
      return owner ? publicUser(owner, this.config.defaultCodexThreadLimit) : null;
    });
  }
