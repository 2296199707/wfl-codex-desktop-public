...
export class PersistentSshToolService {
  constructor({
    ...
    this.capabilities = typeof capabilities === "function" ? capabilities : () => ({ enabled: true });
    this.list = list;
  ...
  async executeRequest(line) {
    ...
    if (request.action === "capabilities") return this.capabilities();
...
// The socket transport is shared by the persistent and temporary WFL SSH
// plugins; each plugin still has its own MCP adapter and authorization handler.
export class TemporarySshToolService extends PersistentSshToolService {}
