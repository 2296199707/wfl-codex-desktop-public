...
export class ThreadWriteLeaseStore {
    ...
    for (let attempt = 0; attempt < 12; attempt += 1) {
      ...
      try {
        return await this.#withLock(leaseDirectory, async () => {
          ...
          if (holders.length && current.ownerId !== ownerId) {
            throw conflictError(publicLease(current, holders), { ownerId: current.ownerId, holders });
          }
          const lease = this.#newHolder(threadId, ownerId, details);
  ...
  async reclaim(threadId, expectedIdentity) {
    validateId(threadId, "Thread id");
    if (!validLeaseIdentity(expectedIdentity)) return false;
    const leaseDirectory = this.#leaseDirectory(threadId);
    try {
      return await this.#withLock(leaseDirectory, async () => {
        const current = await this.#readStoredLease(leaseDirectory);
        const holders = liveHolders(current, this.now());
        if (!sameLeaseIdentity(current, holders, expectedIdentity)) return false;
        await this.#writeLease(leaseDirectory, storedLease(threadId, current.ownerId, []));
        return true;
      });
    } catch (error) {
      if (error.code === "ENOENT") return false;
...
function conflictError(lease, identity = null) {
  ...
  Object.defineProperty(error, "leaseIdentity", {
    value: validLeaseIdentity(identity) ? identity : null,
    enumerable: false,
  });
  return error;
}
...
function validLeaseIdentity(value) {
  return Boolean(
    value
    && typeof value.ownerId === "string"
    && Array.isArray(value.holders)
    ...
    && value.holders.every((holder) => (
      ...
      && Number.isFinite(holder.acquiredAt)
      && Number.isFinite(holder.renewedAt)
      && Number.isFinite(holder.expiresAt)
    )),
  );
}

function sameLeaseIdentity(current, holders, expected) {
  if (!validStoredLease(current) || !validLeaseIdentity(expected)) return false;
  if (current.ownerId !== expected.ownerId || holders.length !== expected.holders.length) return false;
  const expectedByToken = new Map(expected.holders.map((holder) => [holder.token, holder]));
  return holders.every((holder) => {
    const match = expectedByToken.get(holder.token);
    return Boolean(
      match
      && match.acquiredAt === holder.acquiredAt
      && match.renewedAt === holder.renewedAt
      && match.expiresAt === holder.expiresAt,
    );
  });
}
