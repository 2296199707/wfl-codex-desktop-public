...
export function publicCodexNotification(rawPayload, knownMethods = new Set()) {
  ...
  if (method === "modelProvider/authRecoveryStarted" || method === "modelProvider/authRecoveryCompleted") {
    return trustedNotification(method, {
      threadId: requiredIdentifier(params.threadId, "threadId"),
      turnId: requiredIdentifier(params.turnId, "turnId"),
      provider: boundedText(params.provider, 256),
      message: boundedText(params.message, 4_000),
    });
  }
...
function publicGuardianAction(value) {
  ...
  if (type === "writeStdin") {
    return {
      type,
      approvalId: boundedText(action.approvalId, 256),
      processId: boundedText(action.processId, 256),
      cwd: boundedText(action.cwd, 4_096),
      stdin: boundedText(action.stdin, 16_000),
    };
  }
  if (type === "applyPatch") {
    return {
